...
承認ダイアログの**コメント欄に書いた文章は、投稿の先頭に差し込まれます。**

```
【PR】
これ、会議用に買ったけど散歩でも使ってる。   ← 承認時に書いた一言

JBL ワイヤレスイヤホン WAVE BEAM 2
...
「なぜ良いと思ったか」「誰に向いていそうか」は、
商品を使ったことのないプログラムには書けません。書かせれば嘘になりますし、
楽天が禁止している「内容の薄い投稿」そのものになります。
そこだけは人が書く、という分担にしています。

既定では `config.commentReserve` が `0` なので、自動生成の本文が500文字をいっぱいまで使います。
毎回コメントを書きたい場合は、この値を `110` などにすると差し込む余白が確保されます。

**空欄のまま承認して構いません。** その場合はコメントが入らないだけです。

### 8. 公開リポジトリは60日間活動が無いとスケジュール実行が止まる

GitHubの仕様で、公開リポジトリに60日間活動が無いと、スケジュール実行のワークフローは
自動で無効化されます。**このときは赤い×も出ず、静かに止まります。**

承認して投稿するたびに紹介済みリストがコミットされますが、
60日間1度も承認しなかった場合はトークン更新まで止まり、その60日後にトークンが永久に失効します。

これを防ぐため、トークン更新（月2回）のたびに `state/last-refresh.txt` をコミットしています。

もし通知が何日も来ない場合は、Actions タブで各ワークフローが無効化されていないか確認し、
無効になっていれば「Enable workflow」で再開してください。
